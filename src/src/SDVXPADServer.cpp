...
SDVXPADServer::SDVXPADServer(int port)
    : m_impl(std::make_unique<Impl>(port)){};

SDVXPADServer::~SDVXPADServer() = default;

void SDVXPADServer::start_server()
{
    m_impl->start_server_async();
}

void SDVXPADServer::stop_server()
{
    m_impl->stop_server();
}

uint64_t SDVXPADServer::get_controller_state()
{
...
SDVXPADServer::Impl::Impl(int port) : m_port(port),
                                         m_uws_loop(nullptr),
                                         m_uws_socket_token(nullptr),
                                         m_thread(),
                                         m_running(false){};

SDVXPADServer::Impl::~Impl()
{
    if (m_running)
    {
        stop_server();
...
void SDVXPADServer::Impl::start_server_async()
{
    m_thread = std::thread([&] { start_server(); });
}

void SDVXPADServer::Impl::start_server()
{
...
void SDVXPADServer::Impl::stop_server()
{
